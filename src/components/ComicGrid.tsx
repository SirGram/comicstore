  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
      {items.map((item, index) => {
        ...
        // Determine the margin top class
        const marginTopClass = index % 4 === 1 || index % 4 === 3 ? "mt-4" : "";

        return (
          <div
            key={item.id}
            className={`comic relative group h-80 w-56 ${marginTopClass}`}
          >
            <CardImage path={imageUrl} title={item?.title} showShadow={false} />
            <button
              className="absolute inset-x-0 top-0 h-48 cursor-pointer"
            ...
            <div className="absolute -inset-x-1 -bottom-1 h-32 bg-black text-white transform translate-y-full group-hover:translate-y-0 transition-transform duration-300 ease-in-out">
              <button
                className="w-full cursor-pointer bg-mainAccent hover:opacity-80 flex justify-center items-center text-2xl p-3 border-t-2 border-border"
