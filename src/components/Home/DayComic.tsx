...
export default function DayComic({ comic }: { comic: IComic }) {
  return (
    <div className="flex flex-col justify-center items-center  w-full p-10 relative">
      ...
      <h5 className="w-80 text-center bg-secondary comic p-3 -mb-2 z-10">
        {comic.title}
      </h5>
      <article className="flex flex-col gap-4 comic w-80 relative group   bg-center">
        <Link href={`/store/${comic.id}`}>
          <div className="relative w-full h-full">
            <h2 className="inset-0 h-full absolute bg-white justify-center items-center flex bg-opacity-80 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
              ${comic.price}
            </h2>
            <CardImage
